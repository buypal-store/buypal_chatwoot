...
state.cart.forEach(item => {
    ...
    if (item.isMemory) {
        ...
        row.innerHTML = `
          <td style="padding:6px 3px; border-bottom:1px solid var(--border); font-size:10px; color:var(--muted); word-break:break-all;">
            ${item.sku}
          </td>
          <td style="padding:6px 3px; border-bottom:1px solid var(--border);">
            ${item.nombre}
            <div style="display:inline-flex; align-items:center; margin-left:6px; gap:4px;">
              <button class="memoria-downgrade" data-cart-id="${item.cartId}"
                style="width:22px; height:22px; border-radius:50%; border:1px solid #ccc; background:#f9f9f9; cursor:pointer; font-weight:bold; padding:0;">−</button>
              <span class="memoria-capacidad" style="font-weight:600; min-width:44px; text-align:center;">${item.memoryCapacity}</span>
          ...
          <td style="padding:6px 2px; border-bottom:1px solid var(--border); text-align:center;">
            <button class="btn-eliminar-item" data-cart-id="${item.cartId}"
              style="cursor:pointer; font-size:14px; background:none; border:none; color:#ef4444; font-weight:700; padding:0;"
  ...
  // Cantidad por línea en el resumen (aplica también a las memorias de regalo)
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:16px;padding:28px;width:90%;max-width:400px;box-shadow:0 12px 40px rgba(0,0,0,.15);">
      ...
      <div style="display:flex;flex-direction:column;gap:12px;">
        <input id="nuevoSKU" placeholder="SKU" style="padding:10px 12px;border:1px solid #ddd;border-radius:8px;font-size:14px;">
        ...
        <input id="nuevoPrecio" type="number" placeholder="Precio" min="0" style="padding:10px 12px;border:1px solid #ddd;border-radius:8px;font-size:14px;">
